...
拼音学习步骤 1（识记）、步骤 2（练习拼读）、步骤 3（听力测试）目前只围绕拼音项本身
（如声母 `j`）：例字下方/拼读行显示 `{item.pinyin} → {ex}`（如 `j → 家`），步骤3
听力测试的选项也是单个声母/韵母。孩子看不到例字的**完整拼音**（家 = jiā），也无法把
"看到的音节"与"听到的发音"对应起来。

目标：
1. 例字显示其**完整拼音（带声调）**，如 `家 jiā → 家`、步骤1 例字下方 `jiā`。
2. 步骤3 听力测试作答改为**例字的完整拼音**（`家 -- j` → `家 -- jiā`）。
3. 朗读即该字的完整拼音（拼读），显示与声音一致。
4. 消除多音字造成的"显示读音 ≠ 单独朗读声音"的不一致。

用户已确认：带声调显示；步骤1、步骤2、步骤3 都改；多音字用**替换无歧义例字**处理。
...
- **PinyinStep3** 听力测试：作答选项与正确答案改为例字的**完整拼音**。
  - 正确项 = `getCharPinyin(exampleChar)`（家 → jiā）。
  - 错误选项从 `CHAR_PINYIN` 的全部唯一拼音池中取 3 个不等于正确项的拼音，
    与正确项混合后随机排列。
  - 例字展示与朗读按钮保持不变（播放例字音频 = 完整拼音音）。
  - 判分改为 `option === getCharPinyin(exampleChar)`；`allPinyins` 硬编码池移除。
- 朗读按钮保持 `<SpeakAudio text={ex} kind="char" dir="zh" map={pinyinAudioMap} />`
  （播放例字音频 = 完整拼音音；因多音字已替换，显示与声音一致）。
- 引入 `getCharPinyin` 供三个步骤使用。
...
2. 步骤3 听力测试作答选项为完整拼音（家 → jiā），判分正确。
3. 朗读按钮播出的声音与该字完整拼音一致（多音字已替换）。
4. 多音字 乐/了/只 已替换为 悦/拉/猪/直，其音频在映射中并存在。
5. `char-pinyin.test.ts` 全绿；既有测试套件全绿。
6. 生产服务器重建后页面/音频/登录正常。
